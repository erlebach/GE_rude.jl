...
function plot_solution(θ0, θi, p_system, tspan, σ0, p_giesekus; dct)
    ...
    plots = []
    halt = false
    for k = range(1,length(protocols))  # ORIGINAL
        ...
        ppt!(du,u,p,t) = dudt_ppt!(du,u,p,t,protocols[k])
        prob_PTT = ODEProblem(ptt!, σ0, tspan, p_giesekus)
		sol_PTT = solve(prob_PTT,Tsit5(),saveat=dct[:saveat])
        ...
        plot_data!(plots, target[k], target_titles[k], sol_ude_pre, sol_ude_post, sol_giesekus, sol_PTT)
    end
