...
function plot_callback(θ, l, θ0, σ0, p_giesekus, step; dct)
    ...
    halt = false  
    push!(losses, l)
	count_iter = length(losses)
	println("callback_static, iter: $count_iter, loss: $l")
	nb_iter = length(losses)
    ...
    if nb_iter % step == 0
        ...
        fig_file_name = "callback_plot_$nb_iter"
...
function single_run(dct)
    ...
    # println(keys(dct))
    # println(dct[:γ_protoc])
    # println(dct[:ω_protoc])
    # Modify the choice of protcol from `rude-script.jl`

    #=
    v21_1(t) = dct[:γ_protoc][1]*γ*cos(dct[:ω_protoc][1]*ω*t)
    v21_2(t) = dct[:γ_protoc][2]*γ*cos(dct[:ω_protoc][2]*ω*t)
    v21_3(t) = dct[:γ_protoc][3]*γ*cos(dct[:ω_protoc][3]*ω*t)
    v21_4(t) = dct[:γ_protoc][4]*γ*cos(dct[:ω_protoc][4]*ω*t)
    v21_5(t) = dct[:γ_protoc][5]*γ*cos(dct[:ω_protoc][5]*ω*t)
    v21_6(t) = dct[:γ_protoc][6]*γ*cos(dct[:ω_protoc][6]*ω*t)
    v21_7(t) = dct[:γ_protoc][7]*γ*cos(dct[:ω_protoc][7]*ω*t)
    v21_8(t) = dct[:γ_protoc][8]*γ*cos(dct[:ω_protoc][8]*ω*t)
    =#
    ...
    #v21_protoc = [ (t) -> dct[:γ_protoc][1]*cos(dct[:ω_protoc][1]*t) for i in 1:8]

    # It is quite possible that Zygote (auto differentiation) does not work when defining arrays of lambda functions
    # Taking forever with numerical VJPs. (what is VJP?) 
    # NOT SURE.  Must test with MWE. 

    # GE: How are v11, etc computed? All others are set to zero. 
    #gradv_protoc = [ [v11,v12,v13,  v21_protoc[i], v22,v23,v31,v32,v33] for i in 1:8]
    #protocols = gradv_protoc
    ...
    for k = range(start_at, length(protocols), step=1)
        ...
        step = 50
        cb_plot(θ, loss) = plot_callback(θ, loss, θ0, σ0, p_giesekus, step; dct=dct)
        cb_all(θ, loss) = function ()
           foreach(f->f(θ, loss), (cb_fun, cb_plot))
           return false
        end
        # println("after cb_fun")
        # Callback function closure
        adtype = Optimization.AutoZygote()
        # println("after adtype")
        ...
        #parameter_res = Optimization.solve(optprob, Optimisers.AMSGrad(), callback=cb_fun, 
        #   sensealg = ReverseDiffVJP(true), allow_f_increases=false, maxiters=dct[:maxiters]) # original code
        # Adamsw: https://fluxml.ai/Optimisers.jl/dev/api/#Optimisers.AdamW
        # Weird error occurs here during Optimization.solve
        #println("maxiters= ", dct[:maxiters])
		# The error "Warning: Reverse-Mode AD VJP choices all failed. Falling back to numerical VJPs" is still there with next line. 
        parameter_res = Optimization.solve(optprob, Optimisers.AMSGrad(), abstol=1.e-4, reltol=1.e-4, callback=cb_plot, maxiters=dct[:maxiters]) # 
		# The working 
        #parameter_res = Optimization.solve(optprob, Optimisers.AMSGrad(), abstol=1.e-4, reltol=1.e-4, callback=cb_plot, sensealg = ReverseDiffVJP(true), allow_f_increases=true, maxiters=dct[:maxiters]) # original code
        ...
        #println("after parameter_res, Optimization.solve")
        #global θi = parameter_res.u
        θi = parameter_res.u
    ...
    final_plot = plot(plots..., plot_title="Last training step")
    display(final_plot)
    return final_plot 
end
