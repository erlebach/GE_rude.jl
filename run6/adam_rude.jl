...
# GE: reorder protocols to maximize differences between successive protocols
protocols = [gradv_1, gradv_3, gradv_5, gradv_7, gradv_2, gradv_4, gradv_6, gradv_8]
...
for k = range(start_at, length(protocols), step=1)
    ...
    println("k= ", k)
    parameter_res = Optimization.solve(optprob, Optimisers.AMSGrad(), callback=cb_fun, sensealg = ReverseDiffVJP(true), allow_f_increases=false, maxiters=200)
...
for k = range(1,length(protocols),step=1)  # ORIGINAL
	...
	sol_univ = solve(prob_univ, abstol = 1e-7, reltol = 1e-6, saveat=0.1)
