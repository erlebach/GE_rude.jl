...
model_univ = Flux.Chain(Flux.Dense(9, 32, tanh),
                       ...
                       Flux.Dense(32, 9))
p_model, re = Flux.destructure(model_univ)   # THIS SHOULD NOT WORK!
...
for jj = range(1,length(target),step=1)
    for k = range(1,length(protocols),step=1)
        ...
        if target[jj] == "σ12"
            plot!(solve_giesekus.t[1:2:end],σ12_data[1:2:end], c=:red, linestyle=:solid, ms=2, label="Giesekus", gridlinewidth=1, legend=:topright) 
        ...
        elseif target[jj] == "N1"
            plot!(solve_giesekus.t[1:2:end],N1_data[1:2:end], linestyle=:solid, ms=2, c=:red, label="Giesekus", gridlinewidth=1)
        ...
        elseif target[jj] == "N2"
            plot!(solve_giesekus.t,N2_data, linestyle=:solid, c=:red, ms=2, label="Giesekus", gridlinewidth=1, legend=:topright) 
        ...
        elseif target[jj] == "ηE"
            plot!(solve_giesekus.t,-N2_data-N1_data, c=:red, linestyle=:solid, ms=2, label="Giesekus", gridlinewidth=1, legend=:bottomright) 
...
savefig(final_plot, "giesekus_test.pdf")
