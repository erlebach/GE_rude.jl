...
function single_run(dct)
	...
	#σ0 = SA[0f0, 0f0, 0f0, 0f0, 0f0, 0f0, 0f0, 0f0, 0f0]  # now solving 9 equations,  static array
	#σ0 = SA[0., 0., 0., 0., 0., 0., 0., 0., 0.]  # now solving 9 equations,  static array
    ...
    for k = range(1, length(protocols), step=1)
		# protocols[k][4] is v21(t)
		dudt(u, p, t) = dudt_giesekus_opt(u, p, t, protocols[k][4])
        prob_giesekus = ODEProblem(dudt, σ0, tspans[k], p_giesekus)
		# ERROR: matrix(3,3) versus matrix(9,1)
		#solve_giesekus = solve(prob_giesekus, Rodas4(), saveat=dct[:saveat])  # <<< ERROR??
		#solve_giesekus = solve(prob_giesekus, Rodas4())  # does not work
		solve_giesekus = solve(prob_giesekus, Tsit5(), saveat=dct[:saveat])  # works
	...
	#----------------------- MODEL ---------
    model_univ = Flux.Chain(Flux.Dense(dNN[:in_layer], hid, act),
                        Flux.Dense(hid, hid, act),
                        #FastDense(hid, hid, act),  # a second hidden layer
                        Flux.Dense(hid, dNN[:out_layer]))
